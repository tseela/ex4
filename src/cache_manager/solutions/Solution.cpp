...
void Solution::writeToOutputFile(Problem* problem) const {
...
void Solution::writeToOutputFile(Problem* problem, const string& content) const {
    if (m_outputFilePath.compare(NOT_INITIALIZED) == 0) {//no output file
        return;
    }
...
string Solution::getSolutionString(Problem* problem) const {
